...
Param
(
    [Parameter(Position = 0, Mandatory, ParameterSetName = "DataBase")]
    [String]$Database,
...
Select-Object LastLogonTime,DisplayName,ItemCount,
    ...
    ServerName,DatabaseName,  @{label='Type';expression={$_.mailboxtypedetail -replace 'mailbox',''}} |  
Sort-Object "Total Item Size(MB)" -Descending | 
Export-Csv -NoTypeInformation -Encoding Unicode -Path $Path -Delimiter $delimiter

## Archive mailboxes
if ($Database){ 
    $mbx = get-mailbox -Archive -database $Database -ResultSize Unlimited 
} else {
    $mbx = get-mailbox -Archive -server $Server -ResultSize Unlimited 
}
$mbx| 
...
Select-Object LastLogonTime,DisplayName,ItemCount,
    ...
    ServerName,DatabaseName, @{label='Type';expression={$_.mailboxtypedetail -replace 'mailbox',''}} | 
Sort-Object "Total Item Size(MB)" -Descending | 
Export-Csv -NoTypeInformation -Encoding Unicode -Path $Path -Delimiter $delimiter -Append
