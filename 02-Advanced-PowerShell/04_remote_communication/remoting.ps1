...
# Eller ved brug af Pipeline tankegang: 
# $login = (Get-Content .\pc.txt | New-PSSession -ErrorAction SilentlyContinue)

$login

#connect-wsman -computername $PC2
#connect-wsman -computername $PC3


#########################################  one 2 one  ###############
# only one2one remoting using Enter-PSSession
Enter-PSSession -ComputerName $PC1
Enter-PSSession -ComputerName $PC2
Enter-PSSession -ComputerName $PC3

#Enter-PSSession -Session $login[0]
...
Invoke-Command -Session $login -ScriptBlock {netstat}
Invoke-Command -Session $login -ScriptBlock {ipconfig}
Invoke-Command -Session $login -ScriptBlock {Restart-Computer -Force}
Invoke-Command -Session $login -ScriptBlock {stop-Computer -Force}

Invoke-Command -Session $login -ScriptBlock `
{
    Write-Host "PC_Name=$Env:COMPUTERNAME Antal_CPU=$Env:NUMBER_OF_PROCESSORS" 
